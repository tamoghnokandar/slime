...
[startup_optimizations_summary.md](startup_optimizations_summary.md) for what
changed, why it helped, and per-optimization speedups.

> **Note:** [startup_optimizations_summary.md](startup_optimizations_summary.md)
> is a standalone file (same directory as this report). Open it directly in the
> editor; the SGLang and Megatron subsections are linked below as file paths.
...
See [SGLang optimizations](startup_optimizations_summary.md#sglang-optimizations) in
[startup_optimizations_summary.md](startup_optimizations_summary.md) for the full list of SGLang changes and measured speedups.
...
See [Megatron optimizations](startup_optimizations_summary.md#megatron-optimizations) in
[startup_optimizations_summary.md](startup_optimizations_summary.md) for the problem statement, CPU-helper design, and speedup tables.
...
### `megatron_actor_init_wait` sub-phase breakdown (2026-05-20)

Full training path profiled with
[`scripts/benchmark-qwen3-4b-megatron-init-profile.sh`](scripts/benchmark-qwen3-4b-megatron-init-profile.sh)
(**no `--debug-rollout-only`**). See
[startup_optimizations_summary.md](startup_optimizations_summary.md) for the full
table, ref-load evaluation, and optimization opportunities.

| Sub-phase | Time | Share of `init_wait` (38.7s) |
| --- | ---: | ---: |
...
| Untimed (`TrainRayActor.init`, weight updater setup, etc.) | ~18.7s | ~48% |

Qwen3-4B runs use `--use-kl-loss --kl-loss-coef 0.00`, which forces
`with_ref=True` and pays the **~5.5s** ref checkpoint load even though KL loss
weight is zero. Skipping ref requires a paired train-path change, not just a
`with_ref` gate tweak.
...
**Caveat:** e2e `megatron_actor_init_wait` used `--debug-rollout-only`, which
skips Megatron `init()`; the **~18s** init_wait there is Ray overhead only. Full
training `init_wait` is **~39s** (see Megatron sub-phase breakdown above).

Megatron detail inside `megatron_actor_init` (e2e benchmark; init body skipped):
